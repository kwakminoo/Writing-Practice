...
## 예정사항
🛠️ KoNLPy 활용 피드백 시스템 구현 방안KoNLPy는 한국어 텍스트를 형태소 단위로 분석하는 데 탁월하며, 이를 통해 글쓰기의 정량적 분석이 가능해집니다.1. 문장 구조 및 가독성 분석KoNLPy를 이용해 문장 단위로 분리하고 형태소를 분석하여 글의 가독성 지표를 측정합니다.피드백 항목KoNLPy를 활용한 구현 방법제공 피드백 예시평균 문장 길이1. 텍스트를 문장 단위로 분리 (Sentence Splitting). 2. 문장별 어절(띄어쓰기 단위) 또는 형태소(단어) 개수를 계산하여 평균을 냄."평균 문장 길이가 XX개로, 독자가 숨 쉬기 어렵습니다. 문장을 쪼개세요."긴 문장 비율평균 길이보다 긴 문장(예: 30어절 이상)의 비율을 계산."전체 글의 15%가 과도하게 긴 문장입니다. 이 문장들을 짧게 끊어주세요."전문 용어 지수 (가독성)명사 중 한자어 또는 전문 용어의 비중을 측정 (이 지표는 사용자 정의 사전이 필요할 수 있음)."전문 용어 사용 빈도가 높아 일반 독자의 이해가 어려울 수 있습니다. 쉬운 단어로 대체하세요."2. 어휘 다양성 및 반복 분석 (스타일)글의 표현력이 얼마나 풍부한지, 또는 특정 단어를 과도하게 반복하는지 분석합니다.피드백 항목KoNLPy를 활용한 구현 방법제공 피드백 예시어휘 다양성 지수 (TTR)텍스트 내 전체 단어 수 대비 고유한 단어 수의 비율 (Type-Token Ratio)을 계산."어휘 다양성 지수(TTR)가 낮습니다. 반복되는 단어를 피하고 다양한 표현을 사용해보세요."반복 단어/어구 찾기명사, 동사, 형용사 등 주요 품사별로 사용 빈도 상위 10개를 추출하여 표시."‘문제’, ‘있다’, ‘생각하다’ 단어가 지나치게 반복됩니다. 유의어로 대체해 글에 생기를 더하세요."추상적 명사 비율'개념', '사실', '결과' 등 추상적인 명사 사용 비율을 측정."추상 명사 사용 비율이 높아 글이 모호해질 수 있습니다. 구체적인 사례나 묘사를 추가하세요."3. 구조적 피드백 구현글의 형식을 분석하여 구조적 오류를 지적합니다.피드백 항목KoNLPy를 활용한 구현 방법제공 피드백 예시피동/수동태 지수동사 중 피동형(예: ~되다, ~해지다)의 비율을 측정."피동 표현이 많아 글의 활력이 떨어집니다. 능동적인 표현으로 수정하여 주체를 명확히 하세요."주어-서술어 호응문장 내 주어(명사구)와 서술어(동사/형용사)를 추출한 뒤, 자주 발생하는 호응 오류 패턴(DB)과 비교 분석."주어 'A'와 서술어 'B'의 호응이 어색합니다. 문장 구조를 재검토하세요."
