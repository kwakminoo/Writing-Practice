...
const supabase = createClient(
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp2aGhqcm9pZG5wdXh4aHNrZmZ6Iixicm9sZSI6ImFub24iLCJpYXQiOjE3NTI3NDgyNDksImV4cCI6MjA2ODMyNDI0OX0.29sSyISNUcFAUPiZEe96lsJg1kTLwciQUUGQu0s0hYg'
);

async function fixStoryRestructureData() {
  console.log('=== 이야기 재구성 데이터 수정 시작 ===\n');
  
  try {
    // 1. CSV 파일에서 올바른 데이터 읽기
    console.log('1. CSV 파일에서 올바른 데이터 읽는 중...');
    const csvData = [];
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream('이야기 재구성 데이터.csv')
        ...
        .on('data', (data) => {
          ...
          if (data.keywords && data.keywords.trim() !== '') {
            try {
              ...
              console.error(`Keywords 파싱 오류 (ID: ${data.id}):`, e.message);
              // 쉼표로 구분된 문자열로 처리
        ...
        .on('end', async () => {
          console.log(`총 ${csvData.length}개의 데이터를 읽었습니다.\n`);
