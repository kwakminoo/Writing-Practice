...
async function removeAllTemplateData() {
  console.log('=== 모든 타입의 템플릿 문구 및 의미 없는 데이터 삭제 ===\n');
  ...
  const templatePatterns = [
    ...
    // 5감각 묘사 연습 관련 템플릿
    /아래 키워드를 활용해 묘사해보세요/i,
    /다음 키워드를 활용해 묘사해보세요/i,
    /아래 키워드를 활용해 묘사해보세여/i,
    /다음 키워드를 활용해 묘사해보세여/i,
    /아래 키워드를 활용해 묘사해보시오/i,
    /다음 키워드를 활용해 묘사해보시오/i,
    
    // 한 문장 소설 관련 템플릿
    /아래 주제로 한 문장 소설을 써보세요/i,
    /다음 주제로 한 문장 소설을 써보세요/i,
    /아래 주제로 한 문장 소설을 써보세여/i,
    /다음 주제로 한 문장 소설을 써보세여/i,
    /아래 주제로 한 문장 소설을 써보시오/i,
    /다음 주제로 한 문장 소설을 써보시오/i,
    ...
    // 너무 짧거나 의미 없는 프롬프트
    /^[가-힣\s]{1,15}$/, // 15자 이하의 한글만
    /^[a-zA-Z\s]{1,25}$/, // 25자 이하의 영문만
  ...
  allData.forEach((item) => {
    ...
    // 추가 체크: 너무 짧은 프롬프트 (25자 이하)
    if (prompt.length <= 25 && !badData.find(d => d.id === item.id)) {
      badData.push({
        id: item.id,
        type: item.type,
        category: item.category,
        prompt: prompt,
        reason: '너무 짧은 프롬프트 (25자 이하)'
      });
    }
  });
  ...
  if (badData.length > 0) {
    console.log('\n=== 삭제할 데이터 샘플 ===');
    badData.slice(0, 15).forEach((item, idx) => {
