...
export default function BoardPage() {
  ...
  const [writeCategory, setWriteCategory] = useState<'notice' | 'event' | 'suggestion' | 'contest' | 'general'>('general');
  ...
  const fetchPosts = async () => {
    try {
      ...
      const res = await fetch(`/api/board?page=${page}&limit=10${categoryParam}`, {
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {showWriteForm && (
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">게시글 작성</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
            <p className="text-gray-600 dark:text-gray-400">작성된 게시글이 없습니다.</p>
          </div>
        ) : (
          <>
                ...
                {posts.map((post) => (
                  <Link
                    ...
                    className={`block p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
                      post.pinned ? 'bg-yellow-50 dark:bg-yellow-900/10 border-l-4 border-yellow-500' : ''
                    }`}
                  >
