...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('4. 코인 확인 완료:', currentCoins);

    // 5. OpenAI API 키 확인
    console.log('5. OpenAI API 키 확인 시작');
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
      console.log('5. 오류: OpenAI API 키 없음');
